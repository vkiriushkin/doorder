...
public class DoubleFrameSingleTopTransomDoor extends DoubleFrameAngledDoor {
    ...
    private void calcPipe20x20() {
        ...
        if (this.getY() >= 1000 && this.getY() <= 2060
            ...
            this.metal_20x20_Price = bd.doubleValue();
                ...
                && this.getX() >= 901 && this.getX() <= 1200) {
            LOGGER.info("Pipe 20x20: Dimensions OK");
            BigDecimal bd = new BigDecimal((4 * (double) this.getX() + 2 * this.getY()) * 1.1 / 1000 * Price.PIPE_20x20.getPriceInUAH());
            bd = bd.setScale(2, RoundingMode.HALF_UP);
            this.metal_20x20_Price = bd.doubleValue();
        } else
    ...
    public void calcSeal(boolean sealNeeded) {
        BigDecimal bd = new BigDecimal(((double) this.getY_1() + this.getX()) * 4 * 1.1 / 1000 * Price.SEAL.getPriceInUAH());
        bd = bd.setScale(2, RoundingMode.HALF_UP);
    ...
    public void calcOuterDecoration(OuterDecorationType outerDecorationType) {
        ...
        switch (outerDecorationType) {
            ...
            case PAINTING_SHAGREEN:
                this.outerDecoration.calcPaintingShagreen(this.getX(), this.getY(), this.getY_1());
                break;
            case PAINTING_ANTIC:
                this.outerDecoration.calcPaintingAntic(this.getX(), this.getY(), this.getY_1());
                break;
            case PAINTING_PF:
                this.outerDecoration.calcPaintingPF(this.getX(), this.getY(), this.getY_1());
                break;
            case ANTI_LAYER:
