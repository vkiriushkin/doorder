...
public class Armour5030DoubleDoorOuterDecoration extends OuterDecoration {
    ...
    public void calcSelfAdhesiveFilm(int x, int y) {
        ...
        if (x >= 800 && x <= 1200) {
            BigDecimal bd = new BigDecimal(1.1 * 1.5 * (double)y / 1000 * Price.SELF_ADHESIVE_FILM.getPriceInUAH()
                    +(((double)x + y) * 2 * 0.18 + (4 * (double)y + 2 * x) * 0.08) * Price.PAINTING_SHAGREEN.getPriceInUAH() / 1000);
    ...
    public void calcPaintingShagreen(int x, int y) {
        LOGGER.info("Start calculating outer decoration");
        clear();
        BigDecimal bd = new BigDecimal((((double)x * y) / 1000000 + (x + y) * 2 * 0.18 / 1000
                + (2 * (double)x + 4 * y) * 0.08 / 1000) * Price.PAINTING_SHAGREEN.getPriceInUAH());
        bd = bd.setScale(2, RoundingMode.HALF_UP);
    ...
    public void calcPaintingAntic(int x, int y) {
        ...
        BigDecimal bd = new BigDecimal((((double)x * y) / 1000000 + ((double)x + y) * 2 * 0.18 / 1000
                + (2 * (double)x + 4 * y) * 0.08 / 1000) * Price.PAINTING_ANTIC.getPriceInUAH());
        bd = bd.setScale(2, RoundingMode.HALF_UP);
    ...
    public void calcPaintingPF(int x, int y) {
        ...
        clear();
        BigDecimal bd = new BigDecimal((((double)x * y) / 1000000 + ((double)x + y) * 2 * 0.18 / 1000
                + (2 * (double)x + 4 * y) * 0.08 / 1000) * Price.PAINTING_PF.getPriceInUAH());
        bd = bd.setScale(2, RoundingMode.HALF_UP);
    ...
    public void calcAntiLayer(int x, int y) {
        ...
        if (x >= 800 && x <= 1200) {
            BigDecimal bd = new BigDecimal(1.25 * (double)y / 1000 * Price.ANTI_LAYER.getPriceInUAH()
                    + (((double)x + y) * 2 * 0.18 + (2 * (double)x + 4 * y) * 0.08) * Price.PAINTING_SHAGREEN.getPriceInUAH() / 1000);
            bd = bd.setScale(2, RoundingMode.HALF_UP);
