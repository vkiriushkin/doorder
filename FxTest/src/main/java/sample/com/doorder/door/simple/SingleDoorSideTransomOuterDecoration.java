...
public class SingleDoorSideTransomOuterDecoration extends OuterDecoration {
    ...
    public void calcSelfAdhesiveFilm(int x, int y) {
        ...
        if (x >= 800 && x <= 1200) {
            selfAdhesiveFilm = 1.1 * 1.5 * y / 1000 * Price.SELF_ADHESIVE_FILM.getPriceInUAH() + ((x + y) * 2 * 0.1 + (2 * x + 4 * y) * 0.08) * Price.PAINTING_SHAGREEN.getPriceInUAH() / 1000;
            totalOuterDecorationPrice += selfAdhesiveFilm;
        }
        if (x >= 1201 && x <= 1400) {
            double material = 1.1 * 2 * y / 1000;
            selfAdhesiveFilm = material * Price.SELF_ADHESIVE_FILM.getPriceInUAH() + ((x + y) * 2 * 0.1 + (2 * x + 4 * y) * 0.08) * Price.PAINTING_SHAGREEN.getPriceInUAH() / 1000;
            totalOuterDecorationPrice += selfAdhesiveFilm;
        }
        if (x >= 1401 && x <= 1950) {
    ...
    public void calcPaintingShagreen(int x, int y) {
        ...
        BigDecimal bd = new BigDecimal(((double)x * y) / 1000000
                + (x + y) * 2 * 0.1 / 1000 + (4 * y + 2 * x) * 0.06 / 1000);
    ...
    public void calcPaintingAntic(int x, int y) {
        LOGGER.info("Start calculating outer decoration");
        clear();
        BigDecimal bd = new BigDecimal(((double)x * y) / 1000000 + (x + y) * 2 * 0.1 / 1000 + (4 * y + 2 * x) * 0.06 / 1000);
        bd = bd.setScale(2, RoundingMode.HALF_UP);
    ...
    public void calcMdf10(int x, int y) {
        ...
        if (y > 2050 && y <= 2350) {
            BigDecimal bd = new BigDecimal((double)x * y / 1000000);
            bd = bd.setScale(2, RoundingMode.HALF_UP);
            mdf10 = bd.doubleValue() * Price.MDF_10.getPriceInUAH() + 80 + ((x + y) * 2 * 0.1
                    +(x + y) * 2 * 0.1 + (4 * y + 2 * x) * 0.08) * Price.PAINTING_SHAGREEN.getPriceInUAH() / 1000;
            totalOuterDecorationPrice += mdf10;
        }
