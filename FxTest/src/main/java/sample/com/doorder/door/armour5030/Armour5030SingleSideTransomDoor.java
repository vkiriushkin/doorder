...
public class Armour5030SingleSideTransomDoor extends Armour5030Door {

    public static final Logger LOGGER = LoggerFactory.getLogger(Armour5030SingleSideTransomDoor.class);
    ...
    private void calcPipe50x30() {
        ...
        if (this.getY() >= 1000 && this.getY() <= 2450 && this.getX() >= 800 && this.getX() <= 2000
                && this.getX_1() <= this.getX() - 150 && this.getX_1() <= 1200) {
            LOGGER.info("Pipe 50x30: Dimensions OK");
            BigDecimal bd = new BigDecimal((((double)this.getX() + this.getY()) * 4 * 1.1 + (2 * x_1 + 3 * this.getY()) * 1.1) / 1000 * Price.PIPE_50x30.getPriceInUAH());
