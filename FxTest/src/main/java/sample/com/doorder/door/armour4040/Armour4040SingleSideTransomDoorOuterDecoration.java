...
public class Armour4040SingleSideTransomDoorOuterDecoration extends OuterDecoration {
	...
	public void calcSelfAdhesiveFilmWithoutTopTransom(int x, int y, int x_1) {
		...
		if (x_1 >= 500 && x_1 <= 890) {
			BigDecimal bd = new BigDecimal(1.1 * (double) y / 1000 * Price.SELF_ADHESIVE_FILM.getPriceInUAH()
					...
					+ (((double)x + y) * 2 * 0.12 + (2 * x + 4 * y) * 0.08) * Price.PAINTING_SHAGREEN.getPriceInUAH() / 1000);
			bd = bd.setScale(2, RoundingMode.HALF_UP);
			selfAdhesiveFilm = bd.doubleValue();
			totalOuterDecorationPrice += selfAdhesiveFilm;
		}
	...
	public void calcPaintingShagreen(int x, int y) {
		...
		BigDecimal bd = new BigDecimal((((double)x * y) / 1000000
				+ ((double)x + y) * 2 * 0.12 / 1000 + (4 * (double)y + 2 * x) * 0.08 / 1000) * Price.PAINTING_SHAGREEN.getPriceInUAH());
		bd = bd.setScale(2, RoundingMode.HALF_UP);
	...
	public void calcPaintingAntic(int x, int y) {
		LOGGER.info("Start calculating outer decoration");
		clear();
		BigDecimal bd = new BigDecimal((((double)x * y) / 1000000 + ((double)x + y) * 2 * 0.12 / 1000
	...
	public void calcPaintingPF(int x, int y) {
		...
		clear();
		BigDecimal bd = new BigDecimal((((double)x * y) / 1000000
				+ ((double)x + y) * 2 * 0.12 / 1000 + (4 * (double)y + 2 * x) * 0.08 / 1000)  * Price.PAINTING_PF.getPriceInUAH());
		bd = bd.setScale(2, RoundingMode.HALF_UP);
	...
	public void calcAntiLayer(int x, int y) {
		...
		if (x >= 800 && x <= 1200) {
			BigDecimal bd = new BigDecimal((1.25 * (double) y / 1000) * Price.ANTI_LAYER.getPriceInUAH()
					+ (((double)x + y) * 2 * 0.12 + (4 * y + 2 * x) * 0.08) * Price.PAINTING_SHAGREEN.getPriceInUAH() / 1000);
			bd = bd.setScale(2, RoundingMode.HALF_UP);
