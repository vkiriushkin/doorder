...
public class Armour5030SingleTwoSideTransomDoorOuterDecoration extends OuterDecoration {
    ...
    public void calcPaintingAntic(int x, int y, int x_1) {
        ...
        clear();
        BigDecimal bd = new BigDecimal((((double) x * y) / 1000000
                + ((double)x + y) * 4 * 0.18 / 1000 + (4 * (double)y + 2 * x_1) * 0.08 / 1000) * Price.PAINTING_ANTIC.getPriceInUAH());
        bd = bd.setScale(2, RoundingMode.HALF_UP);
    ...
    public void calcPaintingPF(int x, int y, int x_1) {
        ...
        clear();
        BigDecimal bd = new BigDecimal((((double) x * y) / 1000000
                + ((double)x + y) * 4 * 0.18 / 1000 + (4 * (double)y + 2 * x_1) * 0.08 / 1000) * Price.PAINTING_PF.getPriceInUAH());
        bd = bd.setScale(2, RoundingMode.HALF_UP);
